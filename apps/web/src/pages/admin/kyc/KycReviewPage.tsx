    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-foreground text-2xl">KYC Review</h1>
      ...
      <div className="flex items-center gap-2 bg-foreground/5 border border-foreground/10 rounded-xl px-3 py-2 max-w-xs">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search user or document type..." className="bg-transparent text-foreground text-sm outline-none flex-1 placeholder:text-foreground/20" />
      </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-foreground/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-foreground/30 text-sm">No KYC documents found</td></tr>
            ) : filtered.map(doc => (
              <motion.tr key={doc.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="hover:bg-foreground/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <p className="text-foreground text-sm">{doc.profiles?.full_name ?? '—'}</p>
                  <p className="text-foreground/30 text-xs">{doc.profiles?.email ?? '—'}</p>
                </td>
                <td className="px-4 py-3">
                  <span className="text-foreground/70 text-sm capitalize">{doc.document_type.replace('_', ' ')}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-foreground/50 text-xs font-mono">{doc.document_number ?? '—'}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-foreground/50 text-xs">{doc.country_of_issue ?? '—'}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-foreground/40 text-xs">{doc.expiry_date ? formatDate(doc.expiry_date, 'short') : '—'}</span>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className="text-foreground/40 text-xs">{formatDate(doc.created_at, 'short')}</span>
                </td>
                <td className="px-4 py-3">
        ...
        {selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              {/* User info */}
              <div className="p-4 rounded-xl bg-foreground/5 border border-foreground/10 space-y-2 text-sm">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={doc.label} className="flex items-center justify-between p-3 rounded-xl bg-foreground/5 border border-foreground/10">
                    <span className="text-foreground/60 text-sm">{doc.label}</span>
              ...
              {selected.status === 'pending' ? (
                <div className="space-y-3">
                  <div>
                    <label className="text-foreground/50 text-xs mb-1.5 block">Rejection Reason (required if rejecting)</label>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-4">
      ...
      <span className="text-foreground capitalize text-right">{value}</span>
    </div>
  );
}
