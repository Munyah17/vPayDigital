    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-foreground text-2xl">Webhook Events</h1>
            <p className="text-foreground/30 text-sm">{total} total · live feed every 15s</p>
          </div>
        </div>
        <button onClick={() => refetch()} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-foreground/5 border border-foreground/10 text-foreground/50 text-sm hover:text-foreground hover:bg-foreground/10 transition-all">
        ...
        {(Object.entries(statCounts) as [string, number][]).map(([s, n]) => (
          <button key={s} onClick={() => { setStatusFilter(statusFilter === s ? '' : s); setPage(1); }}
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-foreground/40 text-xs capitalize">{s}</p>
            </div>
      ...
      {/* Events list */}
      <div className="glass-card overflow-hidden divide-y divide-foreground/[0.03]">
        {isLoading ? (
          ...
          <div className="px-4 py-12 text-center text-foreground/30 text-sm">No webhook events found</div>
        ) : filtered.map(event => (
          <motion.div key={event.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="px-4 py-3 flex items-center gap-4 hover:bg-foreground/[0.02] transition-colors cursor-pointer"
              onClick={() => setExpanded(expanded === event.id ? null : event.id)}>
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_DOT[event.status]}`} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  <span className="text-foreground text-sm font-mono">{event.event_type}</span>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium capitalize ${STATUS_STYLES[event.status]}`}>{event.status}</span>
                  <span className="text-foreground/20 text-xs">{event.source}</span>
                </div>
                <div className="flex items-center gap-3 mt-0.5">
                  <span className="text-foreground/30 text-xs">{formatDate(event.created_at, 'short')}</span>
                  <span className="text-foreground/20 text-xs">{event.attempts}/{event.max_attempts} attempts</span>
                  {event.idempotency_key && (
                    <span className="text-foreground/20 text-xs font-mono truncate max-w-[200px]">{event.idempotency_key}</span>
                  )}
                </div>
              </div>
            ...
            {expanded === event.id && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                exit={{ height: 0, opacity: 0 }} className="border-t border-foreground/5 bg-black/20">
                <div className="px-4 py-4 space-y-3">
                  ...
                  <div>
                    <p className="text-foreground/30 text-xs mb-2 font-medium uppercase tracking-wider">Payload</p>
                    <pre className="text-foreground/60 text-xs font-mono overflow-auto max-h-48 p-3 bg-black/30 rounded-xl">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    ...
                    <div>
                      <p className="text-foreground/30 mb-0.5">Next retry</p>
                      <p className="text-foreground/60">{event.next_retry_at ? formatDate(event.next_retry_at, 'short') : '—'}</p>
                    </div>
                    <div>
                      <p className="text-foreground/30 mb-0.5">Processed at</p>
                      <p className="text-foreground/60">{event.processed_at ? formatDate(event.processed_at, 'short') : '—'}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-foreground/30 text-xs">{total} total events</p>
