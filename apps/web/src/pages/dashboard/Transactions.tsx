    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`p-2 rounded-xl border transition-all ${showFilters ? 'bg-indigo-600/20 border-indigo-500/40 text-indigo-400' : 'bg-white/5 border-white/10 text-white/40 hover:text-white'}`}
        >
      ...
      {filtered.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="glass-card px-4 py-3">
            <p className="text-white/30 text-[10px] uppercase tracking-wider mb-1">Transactions</p>
            <p className="text-white font-bold font-display text-lg">{filtered.length}</p>
          </div>
          <div className="glass-card px-4 py-3">
            <p className="text-white/30 text-[10px] uppercase tracking-wider mb-1">Money In</p>
            <p className="text-emerald-400 font-bold font-display text-lg tabular-nums">
              +{formatCurrency(totalIn, activeWallet?.currency ?? 'USD')}
            </p>
          </div>
          <div className="glass-card px-4 py-3">
            <p className="text-white/30 text-[10px] uppercase tracking-wider mb-1">Money Out</p>
            <p className="text-white font-bold font-display text-lg tabular-nums">
      ...
      <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5">
        ...
        <input
          ...
          className="bg-transparent text-white/70 placeholder:text-white/20 text-sm flex-1 outline-none"
        />
      </div>
      ...
      {isLoading && transactions.length === 0 ? (
        ...
        <div className="space-y-5">
          {grouped.map(([label, txns]) => (
            <div key={label}>
              <p className="text-white/30 text-xs font-medium uppercase tracking-wider mb-2 px-1">{label}</p>
              <div className="glass-card overflow-hidden">
                <ul className="divide-y divide-white/5">
                  {txns.map(t => (
                    <li key={t.id} className="flex items-center gap-3 p-4 hover:bg-white/[0.02] transition-colors">
