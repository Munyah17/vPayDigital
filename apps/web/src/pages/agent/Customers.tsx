    ...
    <div className="p-4 lg:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-white text-2xl">Customers</h1>
            <p className="text-white/40 text-sm">{total} users with your issued cards</p>
          </div>
        </div>
      </div>

      <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 max-w-sm">
        <Search className="w-4 h-4 text-white/30" />
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search by name or email..." className="bg-transparent text-white text-sm outline-none flex-1 placeholder:text-white/20" />
      </div>
      ...
      {isLoading ? (
        ...
        <>
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {filtered.map(c => (
                  <motion.tr key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <td className="px-4 py-3">
                      <span className="text-white/40 text-xs">{formatDate(c.created_at, 'short')}</span>
                    </td>
                    <td className="px-4 py-3" />
                  </motion.tr>
                ))}
          ...
          <div className="flex items-center justify-between">
            <p className="text-white/30 text-xs">{total} total customers</p>
