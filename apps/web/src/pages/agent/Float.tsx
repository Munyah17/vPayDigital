  ...
  return (
    <div className="p-4 lg:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-display font-bold text-white text-2xl">My float</h1>
      </div>

      {floatWallets.length === 0 ? (
        <div className="glass-card p-12 text-center">
          <p className="text-white/40 text-sm">No float wallets configured yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {floatWallets.map((w) => (
            <div key={w.id} className="glass-card p-5">
              <p className="text-white/40 text-xs uppercase tracking-wider">{w.currency} float</p>
              <p className="font-display font-bold text-white text-3xl mt-1">{formatCurrency(w.balance, w.currency)}</p>
