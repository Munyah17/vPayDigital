    ...
    <div className="p-4 lg:p-6 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display font-bold text-white text-2xl">Help & Support</h1>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <a href="mailto:support@vpay.app"
          className="glass-card p-4 flex items-center gap-3 hover:border-white/20 transition-all rounded-2xl">
          <div className="w-9 h-9 rounded-xl bg-indigo-500/20 flex items-center justify-center">
            <Mail className="w-4 h-4 text-indigo-400" />
          </div>
          <div>
            <p className="text-white text-sm font-medium">Email support</p>
            <p className="text-white/40 text-xs">support@vpay.app</p>
          </div>
        </a>
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1.5">Priority</label>
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1.5">Subject <span className="text-red-400">*</span></label>
              <input value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                className="input-field" placeholder="Brief description of your issue" />
            </div>
            <div>
              <label className="block text-white/60 text-sm mb-1.5">Description <span className="text-red-400">*</span></label>
      ...
      {/* Ticket list */}
      <div className="space-y-3">
        {tickets.length > 0 && <h2 className="text-white font-semibold text-sm">Your Tickets</h2>}
        {isLoading ? (
          ...
          <div className="glass-card p-12 text-center rounded-2xl">
            ...
            <p className="text-white/20 text-xs mt-1">Use the button above to get help</p>
          </div>
        ) : tickets.map(ticket => (
          <div key={ticket.id} className="glass-card rounded-2xl overflow-hidden">
            ...
            {expanded === ticket.id && (
              <div className="px-4 pb-4 border-t border-white/5 pt-3">
                <p className="text-white/60 text-sm whitespace-pre-wrap">{ticket.description}</p>
                {ticket.resolved_at && (
