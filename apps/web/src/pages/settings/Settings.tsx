...
function Toggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${enabled ? 'bg-indigo-600' : 'bg-white/15'}`}
      aria-pressed={enabled}
    >
      <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${enabled ? 'right-0.5' : 'left-0.5'}`} />
    </button>
  );
}
    ...
    <div className="p-4 lg:p-6 max-w-2xl mx-auto space-y-4">
      ...
      <Section>
        <div className="space-y-1 mb-4">
          ...
          <p className="text-white/40 text-xs">Manage your account details</p>
        </div>
      ...
      <Section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-white font-medium text-sm">Preferred Currency</p>
        </div>
      ...
      <Section>
        ...
        <button onClick={() => navigate('/profile/kyc')}
          className="w-full flex items-center justify-between py-3 border-b border-white/5 hover:text-white/80 transition-colors">
          <div className="text-left">
            <p className="text-white/80 text-sm">Identity Verification (KYC)</p>
            <p className="text-white/30 text-xs capitalize">{profile?.kyc_status?.replace('_', ' ') ?? 'Not submitted'}</p>
          </div>
        ...
        <div className="flex items-center justify-between py-3">
          <div>
            ...
            <p className="text-white/30 text-xs">{profile?.two_factor_enabled ? 'Enabled' : 'Disabled'}</p>
          </div>
      ...
      <Section>
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
          <code className="text-white font-mono text-lg flex-1 tracking-widest">{profile?.referral_code ?? '—'}</code>
          <button onClick={copyReferral} className="p-2 rounded-lg hover:bg-white/10 text-white/40 hover:text-white transition-all">
            {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4" />}
          </button>
        </div>
        <p className="text-white/30 text-xs mt-2">Share your code to earn referral bonuses when friends sign up.</p>
      </Section>
