...
const FAQS = [
  ...
  },
  {
    ...
    a: 'You can pay with any vPay wallet balance (USD, EUR, GBP), virtual card, or bank transfer. Crypto payments are coming soon.',
  },
];
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div
      className="border border-white/[0.07] rounded-xl overflow-hidden cursor-pointer"
      onClick={() => setOpen(!open)}
    >
      <div className="flex items-center justify-between gap-4 px-5 py-4">
        <p className="text-white/80 font-medium text-sm">{q}</p>
        {open
      ...
      {open && (
        <div className="px-5 pb-4 border-t border-white/[0.06]">
          <p className="text-white/45 text-xs sm:text-[0.8125rem] leading-relaxed pt-3">{a}</p>
        </div>
      )}
    </div>
  );
...
function Cell({ val }: { val: string | boolean }) {
  ...
  return <span className="text-white/65 text-xs sm:text-[0.8125rem]">{val}</span>;
}
    ...
    <div className="min-h-screen bg-[#0a0a16] overflow-x-hidden">
      ...
      <section className="relative pt-24 sm:pt-32 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-10 text-center overflow-hidden">
        ...
        <motion.div {...fadeUp()} className="relative max-w-2xl mx-auto">
          ...
          <h1 className="font-display font-bold text-white mb-4 leading-[1.08]
          ...
          <div className="inline-flex items-center gap-1 bg-white/[0.04] border border-white/[0.08] rounded-full p-1">
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ${!annual ? 'bg-white text-[#0a0a16]' : 'text-white/50 hover:text-white'}`}
            >
              Monthly
            </button>
            <button
              onClick={() => setAnnual(true)}
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center gap-2 ${annual ? 'bg-white text-[#0a0a16]' : 'text-white/50 hover:text-white'}`}
            >
              Annual
            ...
            >
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className={`px-3.5 py-1 rounded-full text-[11px] font-bold text-white ${
              ...
              <div className="mb-5">
                <p className="text-white font-display font-bold text-base mb-1">{plan.name}</p>
              ...
              <div className="mb-6">
                {plan.price.monthly === 0 ? (
                  <div>
                    <span className="font-display font-bold text-[2.25rem] text-white">Free</span>
                    <p className="text-white/30 text-xs mt-0.5">No credit card required</p>
                  </div>
                ) : (
                  <div>
                    <div className="flex items-end gap-0.5">
                      <span className="text-white/40 text-base font-medium self-start mt-1.5">$</span>
                      <span className="font-display font-bold text-[2.25rem] text-white tabular-nums">
              ...
              <Link
                ...
                className={`flex items-center justify-center gap-2 py-2.5 px-5 rounded-xl font-semibold text-sm mb-6 transition-all duration-200 ${
                  plan.highlight
                    ? 'btn-brand'
                    : 'bg-white/[0.05] text-white border border-white/[0.09] hover:bg-white/[0.09]'
                }`}
              >
      ...
      {/* ── Compare table ────────────────────────────────────────────────────── */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-10 bg-white/[0.01] border-y border-white/[0.05]">
        <div className="max-w-4xl mx-auto">
          <motion.div {...fadeUp()} className="mb-10">
            <h2 className="font-display font-bold text-white mb-2 text-2xl sm:text-[1.875rem]">Full feature comparison</h2>
            <p className="text-white/40 text-sm">Everything side-by-side so you can pick the right fit.</p>
          </motion.div>
            ...
            <table className="w-full min-w-[480px]">
              <thead>
                <tr className="border-b border-white/[0.08]">
                  <th className="text-left pb-3 text-white/35 text-xs font-medium w-1/2">Feature</th>
                  {PLANS.map((p) => (
                    <th key={p.name} className="pb-3 text-center">
                      <span className={`text-xs font-semibold ${p.highlight ? 'text-gradient' : 'text-white/60'}`}>{p.name}</span>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {COMPARE_ROWS.map((row, i) => (
                  <tr key={row.feature} className={`border-b border-white/[0.04] ${i % 2 === 0 ? '' : 'bg-white/[0.01]'}`}>
                    <td className="py-3 text-white/55 text-xs sm:text-[0.8125rem]">{row.feature}</td>
        ...
        >
          <div className="flex items-center justify-center gap-2.5 mb-5">
            {[BarChart3, Users, Headphones].map((Icon, i) => (
              <div key={i} className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.07] flex items-center justify-center text-purple-400">
                <Icon className="w-4 h-4" />
              </div>
            ))}
          </div>
          <h2 className="font-display font-bold text-white mb-3 text-2xl sm:text-[1.875rem]">
            Need a custom plan for your team?
          </h2>
          <p className="text-white/45 text-sm sm:text-[0.9375rem] leading-relaxed mb-7 max-w-xl mx-auto">
      ...
      {/* ── FAQ ──────────────────────────────────────────────────────────────── */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-10 bg-white/[0.01] border-t border-white/[0.05]">
        <div className="max-w-2xl mx-auto">
          <motion.div {...fadeUp()} className="mb-10">
            <h2 className="font-display font-bold text-white mb-2 text-2xl sm:text-[1.875rem]">
              Frequently asked questions
            </h2>
            <p className="text-white/40 text-sm">Still unsure? Here are the questions we hear most.</p>
          </motion.div>
