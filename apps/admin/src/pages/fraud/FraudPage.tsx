...
const STATUS_STYLES: Record<string, string> = {
  ...
  false_positive: 'bg-white/5 text-white/40',
};
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-white text-2xl">Fraud Monitoring</h1>
            <p className="text-white/30 text-sm">{total} total flags</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {(['open', 'investigating', 'resolved', 'false_positive'] as const).map(s => (
            <button key={s} onClick={() => { setStatusFilter(statusFilter === s ? '' : s); setPage(1); }}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${statusFilter === s ? STATUS_STYLES[s] + ' border-current' : 'bg-white/5 border-white/10 text-white/40 hover:text-white'}`}>
      ...
      <div className="flex gap-3">
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search user or flag type..." className="bg-transparent text-white text-sm outline-none flex-1 placeholder:text-white/20" />
        </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-white/30 text-sm">No fraud flags found</td></tr>
            ) : filtered.map(flag => (
              <motion.tr key={flag.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <p className="text-white text-sm font-medium">{flag.profiles?.full_name ?? '—'}</p>
                  <p className="text-white/30 text-xs">{flag.profiles?.email ?? flag.user_id?.slice(0, 8)}</p>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/70 text-xs font-mono">{flag.flag_type}</span>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white/50 text-xs">{flag.risk_score ?? 0}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs ${flag.automated ? 'text-blue-400' : 'text-white/40'}`}>
                    {flag.automated ? 'Auto' : 'Manual'}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/40 text-xs">{formatDate(flag.created_at, 'short')}</span>
                </td>
                <td className="px-4 py-3">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-center justify-between">
                <h2 className="font-bold text-white text-lg">Fraud Flag Review</h2>
                <button onClick={() => setSelected(null)} className="p-1.5 rounded-lg hover:bg-white/10 text-white/30 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {selected.status !== 'resolved' && selected.status !== 'false_positive' && (
                <>
                  <div>
                    <label className="text-white/50 text-xs mb-1.5 block">Resolution Notes</label>
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <button onClick={() => updateMutation.mutate({ id: selected.id, status: 'false_positive', notes: resolutionNotes })}
                      disabled={updateMutation.isPending}
                      className="flex-1 py-2 rounded-xl bg-white/5 border border-white/10 text-white/60 text-sm font-medium hover:bg-white/10 transition-all disabled:opacity-50">
                      False Positive
                    </button>
