    ...
    <div className="p-6 lg:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <Section title="Maintenance Mode" description="Put the platform in maintenance mode to block user activity">
        <div className="flex items-center justify-between py-2">
          <div>
            ...
            <p className="text-white/30 text-xs mt-0.5">Users will see the maintenance message</p>
          </div>
        ...
        {activeMaintenance.enabled && (
          <div>
            <label className="text-white/50 text-xs mb-1.5 block">Maintenance Message</label>
      ...
      <Section title="Supported Currencies" description="Currencies available across the platform">
        <div className="flex flex-wrap gap-2">
          {CURRENCIES.map(c => (
            <span key={c.code} className="px-3 py-1 rounded-xl bg-white/5 border border-white/10 text-white/60 text-xs font-mono">{c.code}</span>
          ))}
        </div>
      </Section>
        ...
        <Section title="All Config Keys" description="Raw system configuration from database">
          <div className="space-y-2">
            {((data?.data?.data ?? []) as ConfigEntry[]).map(entry => (
              <div key={entry.id} className="flex items-start justify-between gap-4 py-2 border-b border-white/[0.04] last:border-0">
                <div className="flex-1 min-w-0">
                  <p className="text-white text-xs font-mono">{entry.key}</p>
                  {entry.description && <p className="text-white/30 text-xs mt-0.5">{entry.description}</p>}
                </div>
                <p className="text-white/30 text-xs flex-shrink-0">{new Date(entry.updated_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        </Section>
...
function Toggle({ enabled, onChange }: { enabled: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!enabled)}
      className={`relative w-10 h-5 rounded-full transition-all ${enabled ? 'bg-indigo-600' : 'bg-white/10'}`}>
