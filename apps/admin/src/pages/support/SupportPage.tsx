...
const STATUS_STYLES: Record<string, string> = {
  ...
  waiting_customer: 'bg-white/5 text-white/40',
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-white text-2xl">Support Tickets</h1>
            <p className="text-white/30 text-sm">{total} tickets</p>
          </div>
        </div>
        <div className="flex gap-1 p-1 bg-white/5 rounded-xl">
          {['open', 'in_progress', 'resolved', ''].map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === s ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white'}`}>
      ...
      <div className="flex gap-3">
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search user, subject, #TKT..." className="bg-transparent text-white text-sm outline-none flex-1 placeholder:text-white/20" />
        </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-white/30 text-sm">No tickets found</td></tr>
            ) : filtered.map(ticket => (
              <motion.tr key={ticket.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <span className="text-white/40 text-xs font-mono">{ticket.ticket_number}</span>
                </td>
                <td className="px-4 py-3">
                  <p className="text-white text-sm">{ticket.profiles?.full_name ?? '—'}</p>
                  <p className="text-white/30 text-xs">{ticket.profiles?.email ?? '—'}</p>
                </td>
                <td className="px-4 py-3">
                  <p className="text-white/80 text-sm max-w-[200px] truncate">{ticket.subject}</p>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/50 text-xs capitalize">{ticket.category.replace('_', ' ')}</span>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-lg text-xs font-medium ${STATUS_STYLES[ticket.status] ?? 'bg-white/5 text-white/40'}`}>
                    {titleCase(ticket.status.replace('_', ' '))}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/40 text-xs">{formatDate(ticket.created_at, 'short')}</span>
                </td>
                <td className="px-4 py-3">
          ...
          <div className="glass-card w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/30 text-xs">{selected.ticket_number}</p>
              </div>
            ...
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <p className="text-white/60 text-sm whitespace-pre-wrap">{selected.description}</p>
            </div>
            <div>
              <label className="text-white/50 text-xs mb-1.5 block">Update Status</label>
              <div className="flex gap-2">
