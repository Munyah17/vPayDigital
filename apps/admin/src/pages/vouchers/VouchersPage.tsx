  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center gap-3">
        ...
        <h1 className="font-display font-bold text-white text-2xl">Vouchers</h1>
      </header>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 max-w-xs">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search name, type, issuer..." className="bg-transparent text-white text-sm outline-none flex-1 placeholder:text-white/20" />
      </div>
      ...
      <div className="glass-card overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={9} className="px-4 py-12 text-center text-white/30 text-sm">No voucher batches found</td></tr>
            ) : filtered.map(batch => {
              ...
              return (
                <motion.tr key={batch.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <td className="px-4 py-3">
                    <p className="text-white text-sm font-medium">{batch.name ?? batch.id.slice(0, 8)}</p>
                    <p className="text-white/30 text-xs font-mono">{batch.id.slice(0, 8)}</p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-white/70 text-sm">{batch.profiles?.full_name ?? '—'}</p>
                    <p className="text-white/30 text-xs">{batch.profiles?.email ?? '—'}</p>
                  </td>
                  <td className="px-4 py-3">
                    <div>
                      <span className="text-white/70 text-xs capitalize">{batch.type.replace('_', ' ')}</span>
                      {batch.gift_card_brand && (
                        <p className="text-white/30 text-xs capitalize">{batch.gift_card_brand.replace('_', ' ')}</p>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3"><span className="text-white text-sm">{batch.quantity}</span></td>
                  <td className="px-4 py-3">
                    <span className="text-white text-sm">{formatCurrency(batch.amount_per_voucher, batch.currency)}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-white text-sm font-medium">{formatCurrency(batch.total_cost, batch.currency)}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white/50 text-xs">{batch.redeemed_count}/{batch.quantity}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-xs ${new Date(batch.expires_at) < new Date() ? 'text-red-400' : 'text-white/40'}`}>
                      {formatDate(batch.expires_at, 'short')}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-white/40 text-xs">{formatDate(batch.created_at, 'short')}</span>
                  </td>
                </motion.tr>
              );
            })}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-white/5">
          <p className="text-white/30 text-xs">{total} total batches</p>
    ...
    <div className="space-y-4">
      <div className="glass-card overflow-hidden">
        <div className="px-4 py-3 border-b border-white/5">
          <h3 className="text-white font-medium text-sm">Vouchers by Type</h3>
        </div>
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-12 text-center text-white/30 text-sm">No data yet</td></tr>
              ...
              return (
                <tr key={type} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3"><span className="text-white/70 text-sm">{stats.qty.toLocaleString()}</span></td>
                  <td className="px-4 py-3"><span className="text-white/70 text-sm">{stats.redeemed.toLocaleString()}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
