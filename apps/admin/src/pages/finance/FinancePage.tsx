  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex items-center gap-3">
        ...
        <h1 className="font-display font-bold text-white text-2xl">Finance & Settlements</h1>
      </header>
      ...
      <div className="flex gap-1 p-1 bg-white/5 rounded-xl w-fit">
        {([
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === t.id ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white'}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-3">
        ...
        <div className="ml-auto text-white/30 text-sm self-center">{total} requests</div>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-white/30 text-sm">No payout requests</td></tr>
            ) : payouts.map((p: any) => (
              <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <p className="text-white text-sm">{p.profiles?.full_name ?? '—'}</p>
                  <p className="text-white/30 text-xs">{p.profiles?.email ?? '—'}</p>
                </td>
                <td className="px-4 py-3">
                  <p className="text-white text-sm font-medium">{formatCurrency(p.amount, p.currency)}</p>
                  <p className="text-white/30 text-xs">Fee: {formatCurrency(p.fee, p.currency)}</p>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/60 text-xs capitalize">{p.method?.replace('_', ' ')}</span>
                </td>
                <td className="px-4 py-3">
                  <p className="text-white/70 text-xs">{p.beneficiary_name ?? '—'}</p>
                  <p className="text-white/30 text-xs">{p.beneficiary_bank ?? ''}</p>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/40 text-xs font-mono">{p.reference}</span>
                </td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-lg text-xs font-medium capitalize ${PAYOUT_STATUS_STYLES[p.status] ?? 'bg-white/5 text-white/40'}`}>
                    {p.status}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-white/40 text-xs">{formatDate(p.created_at, 'short')}</span>
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div className="glass-card w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-white">Review Payout</h3>
              <button onClick={() => setReviewing(null)} className="p-1.5 rounded-lg hover:bg-white/10 text-white/30">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div>
              <label className="text-white/50 text-xs mb-1.5 block">Action</label>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button key={opt.s} onClick={() => setActionStatus(opt.s)}
                    className={`flex items-center justify-center gap-1.5 py-2 rounded-xl border text-xs font-medium transition-all ${actionStatus === opt.s ? opt.cls + ' bg-opacity-20' : 'border-white/10 text-white/40'} ${opt.cls}`}>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-white/30 text-sm">{total} settlements</p>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-12 text-center text-white/30 text-sm">No settlements yet</td></tr>
            ) : settlements.map((s: any) => (
              <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <p className="text-white text-xs">{formatDate(s.settlement_period_start, 'short')}</p>
                  <p className="text-white/30 text-xs">→ {formatDate(s.settlement_period_end, 'short')}</p>
                </td>
                <td className="px-4 py-3"><span className="text-white/70 text-sm">{s.total_transactions ?? 0}</span></td>
                <td className="px-4 py-3"><span className="text-white text-sm font-medium">{formatCurrency(s.gross_amount, s.currency)}</span></td>
                ...
                <td className="px-4 py-3"><span className="text-white/50 text-xs">{s.currency}</span></td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-lg text-xs font-medium capitalize ${SETTLEMENT_STATUS_STYLES[s.status] ?? 'bg-white/5 text-white/40'}`}>
                    {s.status}
                  </span>
                </td>
                <td className="px-4 py-3"><span className="text-white/40 text-xs">{formatDate(s.created_at, 'short')}</span></td>
              </motion.tr>
            ))}
          </tbody>
        </table>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div className="glass-card w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-white">Initiate Settlement</h3>
              <button onClick={() => setShowCreate(false)} className="p-1.5 rounded-lg hover:bg-white/10 text-white/30"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-3">
              ...
              <Field label="Notes">
                <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none resize-none focus:border-indigo-500/50"
                  placeholder="Optional notes..." />
              </Field>
            </div>
...
function FXTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-white/30 text-sm">{rates.length} active rate pairs</p>
        <p className="text-white/20 text-xs">Auto-refreshes every 60s</p>
      </div>
      <div className="glass-card overflow-hidden">
        <table className="w-full">
          <thead>
          ...
          <tbody className="divide-y divide-white/[0.03]">
            {isLoading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-12 text-center text-white/30 text-sm">No exchange rates configured</td></tr>
            ) : rates.map((r: any) => (
              <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3">
                  <span className="text-white font-medium font-mono text-sm">{r.from_currency}/{r.to_currency}</span>
                </td>
                <td className="px-4 py-3"><span className="text-white text-sm">{Number(r.rate).toFixed(6)}</span></td>
                <td className="px-4 py-3"><span className="text-white/60 text-sm">{r.mid_rate ? Number(r.mid_rate).toFixed(6) : '—'}</span></td>
                <td className="px-4 py-3">
                  <span className="text-yellow-400 text-sm">{((r.spread_percentage ?? 0) * 100).toFixed(2)}%</span>
                </td>
                <td className="px-4 py-3"><span className="text-white/40 text-xs">{r.provider}</span></td>
                <td className="px-4 py-3"><span className="text-white/40 text-xs">{formatDate(r.fetched_at, 'short')}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
