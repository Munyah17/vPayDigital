    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/30 text-sm">Generate voucher codes from the master float</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="panel space-y-5">
          <h2 className="text-white font-semibold text-sm">Voucher Configuration</h2>
          ...
          <AnimatePresence>
            {form.type === 'gift_card' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                <label className="block text-white/50 text-xs mb-1.5">Brand</label>
                <div className="relative">
                  <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-white/50 text-xs mb-1.5">Quantity (max 100)</label>
              <input
            ...
            <div>
              <label className="block text-white/50 text-xs mb-1.5">Expires in (days)</label>
              <input
          ...
          <div className="rounded-xl bg-white/3 border border-white/5 p-3 flex items-center justify-between">
            <span className="text-white/40 text-sm">Total cost (incl. 1.5% fee)</span>
            <span className="text-white font-bold">{form.currency} {cost}</span>
          </div>
        ...
        <div className="panel space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-white font-semibold text-sm">Issued This Session</h2>
            {issued.length > 0 && (
          ...
          {issued.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-white/20 text-sm">Issued vouchers will appear here</p>
            </div>
          ) : (
            <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
              {issued.map(v => (
                ...
                >
                  <div>
                    <p className="text-white text-xs font-mono font-semibold tracking-widest">{v.code}</p>
